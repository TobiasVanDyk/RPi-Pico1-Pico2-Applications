...
<img src="images/twist2.png" width="40" height="30"/> **Up to eight I²C RGB rotary encoders** can be plugged into the macropad using a 4-way cable and Qwiic connector. It has software support for both pre-coded functions and programmable symbolic link files to macro actions in App folders. If more than one Twist device choose which Twist device to configure and control with the star commands through \*tc\*\*n with n = 0 to 7 where 0 is for the first Twist device connected.  Other Twist starcode functions: \*tf\* = delete file twist \*tf\*nameR=nameL=nameP default is twist1=twist2=twist3 and \*tm\*char code definitions - \*tm\* twistMacro=0x00 Use symbolic link file twist not coded macros. Default volume vV - \*tm\*char vV uU zZ sS xX dD wW code definitions vuzsxdwVUZSXDW coded macro options volume =/-/mute undo/redo zoom +/-/reset scroll +/- lines-= backspace/delete Minimze-All-Windows (using Press) and Wallaper-Next. The [**Sparkfun Qwiic Twist**](https://www.sparkfun.com/sparkfun-qwiic-twist-rgb-rotary-encoder-breakout.html) was used, and connected to the 3v3, Gnd, and GPIO 26 GPIO 27 for SDA SCL. An alternative is the [**Adafruit I2C Stemma QT Rotary Encoder**](https://www.adafruit.com/product/5880). With I²C as the interconnect, the system scales easily to additional rotary encoders
