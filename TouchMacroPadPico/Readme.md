...
A PC Windows-based file manipulation and configuration tool for the Pico Touch LCD in its early development stages is included in the folder [**Serial2PicoApp**](https://github.com/TobiasVanDyk/RPi-Pico1-Pico2-Applications/tree/main/TouchMacroPadPico/Serial2PicoApp). Additional tabs that must still be completed include a tab for associating PC apps with macros. Note that after unzipping the app, running the executable the first time will download and install .Net 8 run times. Pressing keys on the PC app can either press the same key on the TouchLCD, which then through USB HID, send the keypress back to the PC, or execute many of the actions directly from the PC App itself. Start the app by selecting the Pico COM port, then press Open port, press ok twice for the json MathSets, open the Config Tab and browse to the location where the Math0 and Math1-9 sets are located, and then close and reopen the App. Pressing [Open Port] should then load the the Pico's current configuration into the app. After this first start it will remember the COM port and Math location used, and it will then automatically load this configuration every time after the Open port button is pressed. 

Serial2Pico loads the current button definitions, labels, Layouts and Layers configuration from the TouchLCD when the [Open Port] and [Sync Pad-PC] buttons on the Comms tab of Serial2Pico are pressed. This needs a recent version of the TouchLCD firmware, but it can be used with older firmware but then with fewer functions. The current build PC-App-to-Pico-Firmware combination is: [**Serial2Pico57.zip**](https://github.com/TobiasVanDyk/RPi-Pico1-Pico2-Applications/blob/main/TouchMacroPadPico/Serial2PicoApp/Serial2Pico57.zip), and **VolumeMacroPad-Pico2-35IPS-332.zip** in the Build folder of the Pico 2 version). The older version without the new Serial comms start and end marker function is: [**Serial2Pico48.zip**](https://github.com/TobiasVanDyk/RPi-Pico1-Pico2-Applications/blob/main/TouchMacroPadPico/Serial2PicoApp/App/Serial2Pico48.zip), and **VolumeMacroPad-Pico2-35IPS-329.zip** or **VolumeMacroPad-Pico1-35IPS-423.zip** (in the Build folders of the Pico 1 and 2 versions).
