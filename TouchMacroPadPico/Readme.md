...
Serial2Pico loads the current button definitions, labels, Layouts and Layers configuration from the TouchLCD when the [Open Port] and [Sync Pad-PC] buttons on the Comms tab of Serial2Pico are pressed. This needs a recent version of the TouchLCD firmware, but it can be used with older firmware but then with fewer functions. The current build PC-App-to-Pico-Firmware combination is: [**Serial2Pico63**](https://github.com/TobiasVanDyk/RPi-Pico1-Pico2-Applications/blob/main/TouchMacroPadPico/Serial2PicoApp/Serial2Pico63.zip), and the Pico 1 **VolumeMacroPad-Pico1-35IPS-424.zip** and Pico 2 **VolumeMacroPad-Pico2-35IPS-334.zip** in their build folders. 
