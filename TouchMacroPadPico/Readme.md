...
<p align="left">
<img src="images/win11-88.png" height="172" /> 
...
A PC Windows-based configuration tool for the Pico Touch LCD in its early development stages is included in the folder [**Serial2PicoApp**](https://github.com/TobiasVanDyk/RPi-Pico1-Pico2-Applications/tree/main/TouchMacroPadPico/Serial2PicoApp). Additional tabs that must still be completed include a tab for associating PC apps with macros. Note that after unzipping the app, running the executable the first time will download and install .Net 8 run times. Pressing keys on the PC app can either press the same key on the TouchLCD, which then through USB HID, send the keypress back to the PC, or execute the action directly from the PC App itself. Start the app by selecting the Pico COM port, then press Open port, press ok twice for the json MathSets, open the Config Tab and browse to the location where the Math0 and Math1-9 sets are located, and then close and reopen the App. Pressing [Open Port] shpould then load the the Pico's current configuration into the app. After this first start it should remember the COM port and Math location used, and it will then automatically load the configuration every time after the Open port is pressed. 

Serial2Pico loads the current button definitions, labels, Layouts and Layers configuration from the TouchLCD as either raw data or text data, when the Get Config button on the Main tab of Serial2Pico is pressed. This needs a new version of the TouchLCD firmware, but can be used with the older firmware but then with less functions. The current build PC-App-to-Pico-Firmware combination is: [**Serial2Pico51.zip**](https://github.com/TobiasVanDyk/RPi-Pico1-Pico2-Applications/blob/main/TouchMacroPadPico/Serial2PicoApp/Serial2Pico51.zip), and **VolumeMacroPad-Pico2-35IPS-332.zip** in the Build folder of the Pico 2 version). The older version without the new Serial comms start and end marker function is: [**Serial2Pico48.zip**](https://github.com/TobiasVanDyk/RPi-Pico1-Pico2-Applications/blob/main/TouchMacroPadPico/Serial2PicoApp/Serial2Pico48.zip), and **VolumeMacroPad-Pico2-35IPS-329.zip** or **VolumeMacroPad-Pico1-35IPS-423.zip** (in the Build folders of the Pico 1 and 2 versions).
