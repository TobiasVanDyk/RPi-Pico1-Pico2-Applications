...
Testing: Layouts 1,3, and 4 (keys M S T) are available for custom labels and app-switching - currently only one layout at a time active. If more than one layout with App->Checked the L4 before L3 before L1. L1 and L3 will probably have custom label switched off use [Cfg]->[Opt]->[Custom Labels]->Pad[o] to switch it on or send <\*lm\*>, <\*ls\*>, <\*lt\*>. Remember to put macro files inside the /app/ folder that matches the layout i.e. not T01 but S01 or M01 etc. 
