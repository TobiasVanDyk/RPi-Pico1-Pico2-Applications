...
This touch LCD needs no calibration. The default LCD settings are full brightness and when off full blank. Change 
these by pressing the second Pad on the right [k] or the grey Pad, and then press then press [*Cm] until *bb* shows in 
the status line at the LCD bottom. Then press the [345] key twice (4 shows), press [ADD] and then press [EXE]. The 
...
marker and *1e*char for the End marker, or use *2s*charchar or *2e*charchar to change both start and end characters,
or use *1s* or *1e* to reset both to the < > pair, or use *1s,e*000-255 to set to any value between 0 and 255, or use
*1s,e*00-FF to set to any value between 0 and FF, or use *2s,e*000000-255255 to set start and end to any value between 
0 and 255, or use *2s,e*0000-FFFF to set start and end to any value between 0 and FF, such as the 02/03 hexadecimal 
start/stop transmission pair. When it is changed on the Pico change it on the PC App before syncing, and when changed 
on the PC App via <*1s,e*char> instead of using the single <*2s*charchar> command, remember to keep on using < data >,
as the translation to the new start char is done automatically when sending the *command to change the end character. 
Because the Pico Start and End marker settings are saved in the Config1 file, the values will be 0 after loading 
...
first 0xF3) to the PC. For example construct [Fsp]4x[ADD]D[ADD]F[ADD]8[ADD]9[ADD][Snd] and the hex characters 0xDA 
...
M07 white, press [Fsp]3x[ADD] then a01 via [ADD] then [Sav]. File a01 is already saved in flash content 0xE0 0xE1 0x20 
...
(7) Calibration Enable/Disable - Send the starcode *ca* with the Macroeditor to set/clear re-calibration option after 
    a restart to ON or OFF. NB: This touch LCD needs no calibration.
...
(g) To delete all Flash macro and config files use "*de*" Use *ca* to set/clear re-calibration on start. Remove 
    individual macro files with the [Rmv] or rename with [Ren] keys.
...
(Aa) CircuitPython device control and used as input and/or output for Pico macropad and/or PC App. 
...
(1) If the keys still send a *de* command using the second Pad. Then unplug or reset the keypad. This will delete all 
    files. macros and settings but no calibration is needed.
