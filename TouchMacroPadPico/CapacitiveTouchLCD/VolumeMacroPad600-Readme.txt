...
Sketch uses 282768 bytes (27%) of program storage space. Maximum is 1044480 bytes.
Global variables use 63928 bytes (24%) of dynamic memory, leaving 198216 bytes for local variables. Maximum is 262144 bytes.
Resetting COM12
...
If calibration does not run on first start force by runCalibration(); once then after that use loadCalibration(1); 
Connections: Use Pico gpio 26 27 for i2c. Use the connections in User_Setup.h for the rest (same as Waveshare LCDs)
