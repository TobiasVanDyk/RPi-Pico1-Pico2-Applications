...
Multiple libraries were found for "SD.h"
 Used: C:\Users\Tobias\AppData\Local\Arduino15\packages\rp2040\hardware\rp2040\5.4.4\libraries\SD
...
Using library SD at version 2.0.0 in folder: C:\Users\Tobias\AppData\Local\Arduino15\packages\rp2040\hardware\rp2040\5.4.4\libraries\SD 
...
Sketch uses 241036 bytes (11%) of program storage space. Maximum is 2088960 bytes.
...
New changes:
1. Fixed *e0* *e1* logic errors because Volume Mute not disabled
2. Fixed Vol1,Vol3,Vol4,VolOn label logic
3. *pc* or *pc*t send raw or text TouchLCD configuration data to a PC.
4. Fixed Starcodes not-found check=100
5. Used SD.h Arduino-Pico wrapper for old arduino SDlib instead of SDFS.h - could be side-effects but SD has easier setup than SDFS.
   Previous SDFS versions are still available.
5. Consolidate Mouse Controls by *codes - refer to section (m) in manual.h.
...
6. Fixed History now saved after [k] exit and after [EXE] pressed (not after [Sav] and [Snd]
...
7. MacroEditor history added to existing entries instead of replacing them
8. Additional macro processing options through first char 0xF2 and 0xF3. Add 0xF2 or 0xF3 through pressing key [F+N] in MacroEditor:
...
9. Further fixed MacroBuff not cleared now working with [EXE] in MacroEditor
10. Fixed filenumbers > 24 for saved filename construction
11. Fixes for Function keys F1-F24 and all 17 KeyPad keys if they are the first action in the macro to be executed. As a result Macro strings starting with 0xF0, 0xF1, and 0xFF 
    are treated differently (for Function keys, KeyPad keys, and Modifier-byte respectively).   
12. New third option via [UDM] -> "Mod" key in MacroEditor to force the use of the Modifier byte for Control, Shift, Alt and Gui keys, instead of one or more of the 6 available 
    HID simultaneous keycode slots.
13. Added [KPd] key on MacroEditor for adding the 17 KeyPad keys. Read wiki for examples. Read line 886 in manual.h for 3 methods to execute em-dash.
